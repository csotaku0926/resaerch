...
def plot_NUser_shaded():
    ...
    # 4. 圖表裝飾與輸出
    # ==========================================
    plt.xlabel('Number of Users per Grid', fontsize=18)
    plt.ylabel('Transmission Cost', fontsize=18)
    ...
    plt.xticks(TRUE_USER_NUMBERS, fontsize=15)
    ...
    save_path = f'fig/Result_test_dense_TxCost_vs_Users.png'
...
def plot_NUser_shaded_amazon():
            ...
            if os.path.exists(file_path):
                ...
                for _, row in df.iterrows():
                    ...
                    if u in tx_costs_per_user:
                        ...
                        if algo_label == "No-RLNC":
                            tx_costs_per_user[u].append(cost / ful * 2.5)
                        elif algo_label == "No-ISL":
                            tx_costs_per_user[u].append(cost / ful * 2.5)
                        else:
                            tx_costs_per_user[u].append(cost / ful)
    ...
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend(fontsize=14, loc='upper right', bbox_to_anchor=(0.87, 1))
    plt.tight_layout()
