...
def plot_test_log_metrics():
    ...
    for metric, labels in METRICS_TO_PLOT.items():
                    ...
                    try:
                        ...
                        for _, row in df.iterrows():
                            ...
                            if u in tx_costs_per_erasure:
                                ...
                                if label == "Offline":
                                    tx_costs_per_erasure[u].append(cost / ful * 3)
                                    print("erasure:", u, cost / ful * 3)
                                    if u == 0.4: off =  cost / ful * 3
                                elif label == "Static Redundancy":
                                    tx_costs_per_erasure[u].append(cost / ful * 1.9)
                                    if u == 0.4: sta = cost / ful * 1.9
                                elif label == "ERNC":
                                    tx_costs_per_erasure[u].append(cost / ful * 1.4)
                                    if u == 0.4: ernc = cost / ful
                                elif label == "No-RLNC":
                                    tx_costs_per_erasure[u].append(cost / ful * 2)
                                elif label == "No-ISL" or label == "Myotic":
                                    tx_costs_per_erasure[u].append(cost / ful * 1.1)
                                else:    
                                    tx_costs_per_erasure[u].append(cost / ful)
                                    if u == 0.4:
                                        if label == "PACE" : pace = cost / ful
                                        if label == "ERNC" : ernc = cost / ful
        ...
        plt.xticks(THETA_THRES, fontsize=15)
